...
手で入れ直す場合の手順は [docs/update-check.md](docs/update-check.md) にまとめています。

### 更新の確認

新しいバージョンが出ているかを、エディタの読み込み時に自動で[Releases](https://github.com/limit7412/VRCARKitBlendShapeGenerator/releases/latest)へ問い合わせます。
エディタから外部へ通信しますが、送るのはリリース情報の取得要求だけで、1日1回までです。
通信を望まない場合は Preferences > ARKit BlendShape Generator から止められ、止めれば問い合わせを行わず更新の通知も出ません。

知らせるのは安定版が出たときだけです。

`Packages/` 配下（VCC/ALCOM経由）の場合は、新しいバージョンが出ていることをインスペクタで知らせるだけにとどめ、更新はVCC/ALCOMで行います。
`Assets/` 配下（unitypackageから導入）の場合は、通知のダイアログか、インスペクタの先頭に出る「更新する」ボタンからそのまま更新できます。

更新の仕組みと注意点（取り込み前の控え、フォルダを移動している場合の制限など）は [docs/update-check.md](docs/update-check.md) にまとめています。
...
Sideの詳しい扱いや、特定のBlendShapeを生成させない設定、同名マッピングの重複判定は [docs/mapping.md](docs/mapping.md) にまとめています。
...
表情改変で崩れたシェイプキーをビルド時に直すNDMFツールと併用できます。
生成は修正後のシェイプキーから行われるため、まばたきや口の修正がARKit BlendShapeにも反映されます。
対応を確認しているのは次の2つです。

- [Avatar Blink Fix](https://lemoneru.booth.pm/items/7074770) 3.0系（打ち消し修正、ベイク修正の両方）
- [Face BlendShape Fix](https://github.com/Tr1turbo/FaceBlendShapeFix)

Avatar Blink Fixのプレビュー中は、ソースBlendShapeの一覧に `---Avatar Blink Fix---` という名前のシェイプキーが現れます。
修正済みであることを示す目印で、変形を持たないため、ソースには選ばないでください。
Avatar Blink Fixのプレビューを止めているあいだは、本ツールのプレビューだけが修正前の形から生成されます。ビルドでは修正後の形から生成されます。

Face BlendShape Fixのプレビューは、元のシェイプキーを書き換えずに確認用のシェイプキーを別名で足す作りです。
そのため本ツールのプレビューは修正前の形から生成され、修正後の形から生成されるビルド結果とは一致しません。
修正が反映された形を見るには、ビルド結果で確認してください。
